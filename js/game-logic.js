...
window.initTreeSystem = function (THREE, scene, count, currentMap) {
    const isDesert = currentMap === 'desert';

    // Trunk Geometry
    const trunkGeo = new THREE.CylinderGeometry(isDesert ? 0.2 : 0.4, isDesert ? 0.3 : 0.6, 5, 8);
    trunkGeo.translate(0, 2.5, 0); // Origin at bottom
    const trunkMat = new THREE.MeshPhongMaterial({ color: isDesert ? 0xb08d57 : 0x5d4037 });
    ...
    let foliageMat;
    if (isDesert) {
        // Palm leaves style (simplified as a wide flat cone)
        foliageGeo = new THREE.ConeGeometry(3, 1, 8);
        foliageGeo.translate(0, 5, 0);
        foliageMat = new THREE.MeshPhongMaterial({ color: 0x228b22 });
    } else {
        // Pine/Regular tree style (cone)
        foliageGeo = new THREE.ConeGeometry(2.5, 7, 8);
        foliageGeo.translate(0, 6, 0);
        foliageMat = new THREE.MeshPhongMaterial({ color: 0x145214 });
    }
    ...
    return { trunkMesh, trunkIndex: 0, foliageMesh, foliageIndex: 0 };
};

window.createTree = function (THREE, x, z, currentMap, system, buildings) {
    const dummy = new THREE.Object3D();
    ...
    dummy.updateMatrix();
    system.trunkMesh.setMatrixAt(system.trunkIndex++, dummy.matrix);

    // Foliage
    system.foliageMesh.setMatrixAt(system.foliageIndex++, dummy.matrix);

    system.trunkMesh.instanceMatrix.needsUpdate = true;
    system.foliageMesh.instanceMatrix.needsUpdate = true;

    // Add to collision (trunks are narrow, but we give them a decent hit box)
    const trunkWidth = (isDesert ? 0.8 : 1.2) * scale;
    const treeData = { x, z, w: trunkWidth, h: 5 * scale, isTree: true };
    buildings.push(treeData);
    return treeData;
};
...
window.initBuildingSystem = function (THREE, scene, count, conf) {
    ...
    const brickTex = texLoader.load(window.TEXTURE_DATA.brick);
    const concreteTex = texLoader.load(window.TEXTURE_DATA.concrete);
    const metalTex = texLoader.load(window.TEXTURE_DATA.metal);

    // Set up texture wrapping and repeat for proper tiling
    [brickTex, concreteTex, metalTex].forEach(tex => {
        tex.wrapS = tex.wrapT = THREE.RepeatWrapping;
        tex.repeat.set(2, 4); // Repeat vertically more for tall buildings
    });

    const colors = conf.isNight ? [0x0f0f1a, 0x2c3e50, 0x1a1a2e] : [0x7f8c8d, 0x95a5a6, 0xd35400];
    const textures = [brickTex, concreteTex, metalTex];

    const systems = colors.map((c, i) => {
        const mesh = new THREE.InstancedMesh(
            unitGeo,
            new THREE.MeshPhongMaterial({
                map: textures[i],
                color: conf.isNight ? 0x888888 : 0xffffff // Slight tint for night mode
            }),
            count
        );
    ...
    // Windows instance (Night/Day variants) - keep emissive for brightness
...
window.updateEnemies = function (enemies, camera, currentZoneRadius, nextZoneCenter, currentMap, scene, bullets, playSound, createImpact, particles, audioCtx, THREE) {
    ...
    for (let i = enemies.length - 1; i >= 0; i--) {
        ...
        if (en.health <= 0) {
            ...
            const droppedWeapon = en.mesh.userData.weaponType || 'rifle';
            window.loots.push({ mesh: box, type: Math.random() > 0.5 ? 'medkit' : 'weapon', weaponType: droppedWeapon, isDeathBox: true, glow: glow });
